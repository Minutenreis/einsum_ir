...
plt.figure(figsize=(10, 5))
plt.plot(size_1_arr, [new/base for new, base in zip(gflops_144_arr, gflops_72_0_arr)], '-o', label="einsum_ir 144 cores")
plt.plot(size_1_arr, [new/base for new, base in zip(gflops_mpi_c, gflops_72_0_arr)], '-s', label="einsum_ir 144 cores distributed c0")
plt.plot(size_1_arr, [new/base for new, base in zip(gflops_72_1_arr, gflops_72_0_arr)], '-^', label="einsum_ir 72 cores socket 1")
plt.plot(size_1_arr, [new/base for new, base in zip(gflops_mpi_m_n, gflops_72_0_arr)], '-*', label="einsum_ir 144 cores distributed m0 and n0")
plt.plot(size_1_arr, [new/base for new, base in zip(gflops_mpi_k, gflops_72_0_arr)], '-P', label="einsum_ir 144 cores distributed k0")
plt.grid(axis="both")

plt.xlabel("$m_0 = n_0 = k_0$")
plt.ylabel("Speedup")
plt.legend()
plt.title("Average Speedup on NVIDIA Grace CPU Superchip compared to einsum_ir 72 cores socket 0")
