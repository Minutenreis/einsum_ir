...
plt.errorbar(size_1_arr, gflops_arr, yerr=[np.array(gflops_arr) - np.array(gflops_10_arr), np.array(gflops_90_arr) - np.array(gflops_arr)], fmt='r-o', capsize=5, label="einsum_ir")
plt.errorbar(size_1_arr, gflops_mpi_arr, yerr=[np.array(gflops_mpi_arr) - np.array(gflops_mpi_10_arr), np.array(gflops_mpi_90_arr) - np.array(gflops_mpi_arr)], fmt='b-o', capsize=5, label="einsum_ir_mpi")
plt.xlabel("m0 = k0 = k1")
plt.ylabel("GFLOPS")
plt.legend()
...
plt.figure(figsize=(10, 5))
# plt.plot(size_1_arr, speedup_arr, label="Median Speedup")
plt.errorbar(size_1_arr, speedup_arr, yerr=[np.array(speedup_arr) - np.array(speedup_10_arr), np.array(speedup_90_arr) - np.array(speedup_arr)], fmt='r-o', capsize=5, label="Speedup einsum_ir_mpi over einsum_ir")
plt.axhline(y=1, color='black', linestyle='--', label="einsum_ir")
