...
do
	for j in {1..10}
	do
      /home/fedora/mpich/bin/mpirun -n 2 --bind-to numa --map-by numa -env LIBXSMM_TARGET=aarch64 -env OMP_NUM_THREADS=70 ./build/bench_tree_mpi_dist $i 70 c >> grace_dist_c.csv
      /home/fedora/mpich/bin/mpirun -n 2 --bind-to numa --map-by numa -env LIBXSMM_TARGET=aarch64 -env OMP_NUM_THREADS=70 ./build/bench_tree_mpi_dist $i 70 m >> grace_dist_m_n.csv
      /home/fedora/mpich/bin/mpirun -n 2 --bind-to numa --map-by numa -env LIBXSMM_TARGET=aarch64 -env OMP_NUM_THREADS=70 ./build/bench_tree_mpi_dist $i 70 k >> grace_dist_k.csv
