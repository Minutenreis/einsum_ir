...
int main(int i_argc, char *i_argv[]) {
  if (i_argc < 4) {
    ...
    std::cerr << "  ./bench_expression einsum_string dimension_sizes "
                 "contraction_path dtype store_lock print_tree"
              << std::endl;
    std::cerr << std::endl;
    std::cerr << "Arguments:" << std::endl;
    std::cerr << "  * einsum_string:    Einsum expression string. Either in "
                 "single-character or standard format."
              << std::endl;
    std::cerr << "  * dimension_sizes:  Dimension sizes have to be in "
                 "ascending order of the dimension names."
              << std::endl;
    std::cerr << "                      ASCII numbers (see Example #3) are "
                 "sorted by their numeric value."
              << std::endl;
    std::cerr << "  * contraction_path: Contraction path." << std::endl;
    std::cerr << "  * dtype:            FP32, FP64, CPX_FP32 or CPX_FP64, "
                 "default: FP32."
              << std::endl;
    std::cerr << "  * store_lock:       If 1 all einsum_ir input tensors are "
                 "stored and locked before evaluation, default: 0."
              << std::endl;
    std::cerr << "  * print_tree:       If not 0 the einsum tree is printed "
                 "(1: dimension ids, 2: characters), default: 0."
              << std::endl;
    std::cerr << std::endl;
    std::cerr << "Example #1 (single character format):" << std::endl;
    std::cerr << "  ./bench_expression \"iae,bf,dcba,cg,dh->hgfei\" "
                 "\"32,8,4,2,16,64,8,8,8\" \"(1,2),(2,3),(0,1),(0,1)\""
              << std::endl;
    std::cerr << "Example #2 (standard format):" << std::endl;
    std::cerr << "  ./bench_expression "
                 "\"[i,a,e],[b,f],[d,c,b,a],[c,g],[d,h]->[h,g,f,e,i]\" "
                 "\"32,8,4,2,16,64,8,8,8\" \"(1,2),(2,3),(0,1),(0,1)\""
              << std::endl;
    std::cerr << "Example #3 (standard format using integers):" << std::endl;
    std::cerr << "  ./bench_expression "
                 "\"[8,0,4],[1,5],[3,2,1,0],[2,6],[3,7]->[7,6,5,4,8]\" "
                 "\"32,8,4,2,16,64,8,8,8\" \"(1,2),(2,3),(0,1),(0,1)\""
              << std::endl;
    return EXIT_FAILURE;
  }
  ...
  if (l_expression_string_arg[0] == '[') {
    ...
    einsum_ir::frontend::EinsumExpressionAscii::standard_to_schar(
        l_expression_string_std, l_expression_string_schar);
  } else {
    l_expression_string_schar = l_expression_string_arg;
    einsum_ir::frontend::EinsumExpressionAscii::schar_to_standard(
        l_expression_string_schar, l_expression_string_std);
  }
  ...
  einsum_ir::frontend::EinsumExpressionAscii::parse_tensors(
      l_expression_string_std, l_tensors);
  int64_t l_num_tensors = l_tensors.size();

  std::cout << "parsed tensors:" << std::endl;
  for (std::string tensor : l_tensors) {
  ...
  einsum_ir::frontend::EinsumExpressionAscii::parse_dim_sizes(
      l_dim_sizes_string, l_dim_sizes);
  ...
  einsum_ir::frontend::EinsumExpressionAscii::parse_path(l_path_string, l_path);

  std::cout << "parsed contraction path: ";
  for (std::size_t path : l_path) {
    std::cout << path << " ";
  }
  std::cout << std::endl;
  ...
  einsum_ir::frontend::EinsumExpressionAscii::parse_dim_ids(
      l_expression_string_std, m_map_dim_name_to_id);

  std::cout << "parsed dimension ids:" << std::endl;
  for (auto l_di : m_map_dim_name_to_id) {
  ...
  for (int64_t l_te = 0; l_te < l_num_tensors; l_te++) {
    ...
    einsum_ir::frontend::EinsumExpressionAscii::split_string(
        l_tensors[l_te], std::string(","), l_tensor_dim_names);
  ...
  for (std::string l_tensor : l_tensors) {
    std::vector<std::string> l_tensor_dim_names;
    einsum_ir::frontend::EinsumExpressionAscii::split_string(
        l_tensor, std::string(","), l_tensor_dim_names);
  ...
  for (std::size_t string_num_dim : l_string_num_dims) {
    std::cout << string_num_dim << " ";
  }
  std::cout << std::endl;

  std::cout << "  string_dim_ids: ";
  for (std::size_t l_string_dim_id : l_string_dim_ids) {
    std::cout << l_string_dim_id << " ";
  }
  std::cout << std::endl;
  ...
  int64_t l_off = 0;
  for (std::size_t l_string_num_dim : l_string_num_dims) {
    // assemble size of the tensor
    std::vector<int64_t> l_sizes;
    int64_t l_num_dims = l_string_num_dim;
    if (l_ctype_einsum_ir != einsum_ir::REAL_ONLY) {
      l_num_dims -= 1;
    }
    ...
    l_off += l_string_num_dim;

    l_data.push_back(at::randn(l_sizes, l_dtype_at));
  }

  std::vector<void *> l_data_ptrs(l_data.size());
  for (auto &data : l_data) {
    l_data_ptrs.push_back(data.data_ptr());
  }
  ...
  l_einsum_exp.init(l_dim_sizes.size(), l_dim_sizes.data(), l_path.size() / 2,
                    l_string_num_dims.data(), l_string_dim_ids.data(),
                    l_path.data(), l_ctype_einsum_ir, l_dtype_einsum_ir,
                    l_data_ptrs.data());
  ...
  if (l_print_tree == 1) {
    ...
    for (std::map<std::string, int64_t>::reverse_iterator l_di =
             m_map_dim_name_to_id.rbegin();
         l_di != m_map_dim_name_to_id.rend(); l_di++) {
      std::string l_dim_name = l_di->first;
      int64_t l_dim_id = l_di->second;
  ...
  if (l_store_and_lock) {
    for (int64_t l_te = 0; l_te < l_num_tensors - 1; l_te++) {
      ...
      if (l_err != einsum_ir::SUCCESS) {
        std::cerr << "error: failed to store and lock tensor with id: " << l_te
                  << std::endl;
        return EXIT_FAILURE;
      }
    }
  ...
  std::cout << "CSV_DATA: "
            ...
            << "\"" << l_dim_sizes_string << "\","
            << "\"" << l_path_string << "\"," << l_num_flops << ","
            << l_time_compile << "," << l_time_eval << "," << l_gflops_eval
            << "," << l_gflops_total << std::endl;

  /*
   * run at::einsum
   */
  at::Tensor l_out_aten;
  if (m_map_dim_name_to_id.size() <
      53) { // at::einsum does not support more than 52 dimensions
    ...
    // warmup run
    l_out_aten = at::einsum(l_expression_string_schar, l_data_in, l_path);
  ...
  einsum_ir::backend::EinsumNode *l_bin_conts =
      l_einsum_exp.m_nodes.data() + l_num_tensors - 1;
  ...
  for (int64_t l_co = 0; l_co < l_num_tensors - 2; l_co++) {
    ...
    l_c = std::accumulate(l_bin_conts[l_co].m_cont->m_sizes_c.data(),
                          l_bin_conts[l_co].m_cont->m_sizes_c.data() +
                              l_bin_conts[l_co].m_cont->m_num_dims_c,
                          1, std::multiplies<int64_t>());

    l_m = std::accumulate(l_bin_conts[l_co].m_cont->m_sizes_m.data(),
                          l_bin_conts[l_co].m_cont->m_sizes_m.data() +
                              l_bin_conts[l_co].m_cont->m_num_dims_m,
                          1, std::multiplies<int64_t>());

    l_n = std::accumulate(l_bin_conts[l_co].m_cont->m_sizes_n.data(),
                          l_bin_conts[l_co].m_cont->m_sizes_n.data() +
                              l_bin_conts[l_co].m_cont->m_num_dims_n,
                          1, std::multiplies<int64_t>());

    l_k = std::accumulate(l_bin_conts[l_co].m_cont->m_sizes_k.data(),
                          l_bin_conts[l_co].m_cont->m_sizes_k.data() +
                              l_bin_conts[l_co].m_cont->m_num_dims_k,
                          1, std::multiplies<int64_t>());
    ...
    at::Tensor l_mat_a = at::randn({l_c, l_k, l_m}, l_dtype_at);
    at::Tensor l_mat_b = at::randn({l_c, l_n, l_k}, l_dtype_at);

    // warmup run
    at::Tensor l_mat_c = at::matmul(l_mat_b, l_mat_a);

    l_tp0 = std::chrono::steady_clock::now();
    l_mat_c = at::matmul(l_mat_b, l_mat_a);
    l_tp1 = std::chrono::steady_clock::now();
    l_dur = std::chrono::duration_cast<std::chrono::duration<double>>(l_tp1 -
                                                                      l_tp0);
    ...
    std::cout << "    #" << l_co << ": " << l_c << " " << l_m << " " << l_n
              << " " << l_k << std::endl;
  }
  if (l_num_flops_matmul != l_num_flops) {
    std::cerr << "error: flops performed through batched gemms dont match"
              << std::endl;
    return EXIT_FAILURE;
  }
  ...
  if (m_map_dim_name_to_id.size() <
    ...
    std::cout << "  maximum absolute entry in ATen solution:      "
              << at::max(at::abs(l_out_aten)).item() << std::endl;
    std::cout << "  maximum absolute entry in einsum_ir solution: "
              << at::max(at::abs(l_data.back())).item() << std::endl;
    std::cout << "  maximum element-wise difference:              "
              << at::max(at::abs(l_out_aten - l_data.back())).item()
              << std::endl;
    if (!at::allclose(l_out_aten, l_data.back())) {
      std::cerr << "warning: einsum_ir solution is not close to at:einsum!"
                << std::endl;
      return EXIT_FAILURE;
    }
  }
